...
class QaRobotActor extends it.unibo.qactor.robot.RobotActor{
	public QaRobotActor(
		...
		super(name, ctx, "./srcMore/it/unibo/qrdemo0robot/plans.txt", worldTheoryPath,
		outEnvView, it.unibo.qactor.robot.RobotSysKb.setRobotBase(ctx, baserobot) , defaultPlan);
	}
}

public class AbstractQrdemo0robot extends QaRobotActor { 
...
protected IActorAction  action;
	...
	public AbstractQrdemo0robot(String actorId, QActorContext myCtx, IOutputEnvView outEnvView ,String baserobot)  throws Exception{
		super(actorId, myCtx,  
		"./srcMore/it/unibo/qrdemo0robot/WorldTheory.pl",
		setTheEnv( outEnvView ) ,baserobot , "init");		
		this.planFilePath = "./srcMore/it/unibo/qrdemo0robot/plans.txt";
	...
	*/
    ...
    while(true){
    	...
    	nPlanIter++;
    		...
    		if( currentMessage.msgId().equals("sonar") ){
    			...
    			/* Print */
    			parg =  updateVars( Term.createTerm("sonar(SONARNAME,TARGETNAME,DISTANCE)"), Term.createTerm("sonar(sonara,qrdemo0robot,F)"), 
    				    		  					Term.createTerm(currentMessage.msgContent()), parg);
    		...
    		if( currentMessage.msgId().equals("sonar") ){
    			String parg="distA(S)";
    			/* AddRule */
    			parg = updateVars(Term.createTerm("sonar(SONARNAME,TARGETNAME,DISTANCE)"),  Term.createTerm("sonar(sonara,qrdemo0robot,S)"), 
    				    		  					Term.createTerm(currentMessage.msgContent()), parg);
    		...
    		if( currentMessage.msgId().equals("sonar") ){
    			String parg = "";
    			/* SwitchPlan */
    			parg =  updateVars(  Term.createTerm("sonar(SONARNAME,TARGETNAME,DISTANCE)"), Term.createTerm("sonar(sonara,qrdemo0robot,D)"), 
    				    		  					Term.createTerm(currentMessage.msgContent()), parg);
    				if( parg != null ){
    ...
    while(true){
    	...
    	nPlanIter++;
    		...
    		if( currentMessage.msgId().equals("sonar") ){
    			...
    			/* Print */
    			parg =  updateVars( Term.createTerm("sonar(SONARNAME,TARGETNAME,DISTANCE)"), Term.createTerm("sonar(sonarb,qrdemo0robot,F)"), 
    				    		  					Term.createTerm(currentMessage.msgContent()), parg);
    				if( parg != null ) println( parg );  
    		}//onMsg
    		if( currentMessage.msgId().equals("sonar") ){
    ...
    while(true){
    	...
    	nPlanIter++;
    		...
    		println( temporaryStr );  
    		//forward
    		//if( ! execRobotMove("startRobot","forward",60,0,0, "" , "") ) break;
    		    aar = execRobotMove("startRobot","forward",60,0,0, "" , "");
    		    if( aar.getInterrupted() ){
    		    	curPlanInExec   = "startRobot";
    		    	if( ! aar.getGoon() ) break;
    		    } 			
    ...
    while(true){
    	...
    	nPlanIter++;
    		...
    		println( temporaryStr );  
    		//forward
    		//if( ! execRobotMove("leftRobot","forward",60,0,0, "" , "") ) break;
    		    aar = execRobotMove("leftRobot","forward",60,0,0, "" , "");
    		    if( aar.getInterrupted() ){
    		    	curPlanInExec   = "leftRobot";
    		    	if( ! aar.getGoon() ) break;
    		    } 			
    ...
    while(true){
    	...
    	nPlanIter++;
    		...
    		println( temporaryStr );  
    		//forward
    		//if( ! execRobotMove("rightRobot","forward",60,0,0, "" , "") ) break;
    		    aar = execRobotMove("rightRobot","forward",60,0,0, "" , "");
    		    if( aar.getInterrupted() ){
    		    	curPlanInExec   = "rightRobot";
    		    	if( ! aar.getGoon() ) break;
    		    } 			
    ...
    */
    protected void initSensorSystem(){		
    	try {
    		String goal = "consult( \"./src/it/unibo/qrdemo0robot/sensorTheory.pl\" )";
    ...
    */
    protected void addSensorObservers(){
    	for (ISensor<?> sensor : Configurator.getInstance().getSensors()) {
    		//println( "qrdemo0robot sensor= "  + sensor.getDefStringRep() );
    		//println( "qrdemo0robot sensor class= "  + sensor.getClass().getName() );
