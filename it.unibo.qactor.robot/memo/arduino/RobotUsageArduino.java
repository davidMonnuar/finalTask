 ...
 * ----------------------------------------------------------------
 */
public class RobotUsageArduino extends SituatedPlainObject{
	...
	protected void explain(){
		...
		println( "DEFINES: a generic SensorObserver to show the sensor data"    );
		println( "%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%"    );
	}
	...
	protected void addSensorObservers(){
		...
		ISensorObserver<?> obs = new SensorObserver<ISensorData>(outEnvView);
		...
		for (ISensor<?> sensor : configurator.getSensors()) {
			...
			if( sensor instanceof DistanceSensor){
				DistanceSensor sensorDistance = (DistanceSensor) sensor;
 				println( "RobotUsageNaive add the observer to  "  + sensor.getDefStringRep() );
				...
				println( "CtxRobotAvatar add the observer to  "  + sensor.getDefStringRep() );
