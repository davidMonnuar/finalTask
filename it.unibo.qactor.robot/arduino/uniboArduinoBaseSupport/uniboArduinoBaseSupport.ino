...
/*
 * DISTANCE position : f
 * MAGNETOMETER position : t
 * TEMPERATURE postion : t
 * LINE postion : b
 */
...
String buildJsonLine(int r ){
    ...
    //ss = ss + String(r) ;
    if( r == 0 )  ss = ss + "\"lineDetected\"" ; else ss = ss + "\"lineLeft\"" ;
    ss = ss + "},\"tm\":" + millis() + "}";
    return ss;
}
...
int curcx, curcy, curcz;
float curtemper = 0;

void explainGy85(){
  Serial.println("Updated by AN from project GY85 on GITHUB");
  Serial.println("USE: GY_85.h (and GY_85.cpp)"); 
}
void setupGY85()
{
    Wire.begin();
...
void getGY85TemperatureData(boolean json){
    ...
    if( json) {
      //Serial.println(" curtemper "+ String(curtemper) + " gt=" + String(gt) );
      if( abs( curtemper - gt ) > 0.1 ){
...
void jobGY85() {
  //Serial.println("accelerom="+String(accelerom)+ " compass=" + String(compass));
  if( accelerom )         getGY85AccelerometerData();
  else if( gyroscope )    getGY85GyroData();
  else if( compass )      getGY85CompassData(jsonFormat);
  else if( temperature )  getGY85TemperatureData(jsonFormat);
  /*
  else{
...
void job6050(){
  ...
  readRawData();
  if( gyroscope )       getGyroDataForSerialchart();
  else if( accelerom )  getAccelerometerDataForSerialchart(); 
  //delay(133);
}
/*
...
int read_hcsr04(int trig, int echo, int current, String dir){
  ...
  if(r > 4 && r < 150  && abs(current - r) > 0.5 ) {   
 ...
 */
  ...
  */
void elabSensorSelection(int inbyte){
  Serial.println("elabSensorSelection " + String(inbyte) );
  if( inbyte == 'r' ){  //reset
    accelerom   = false;
    gyroscope   = false;
    line        = false;
  ...
  else if( inbyte =='a' ) accelerom = true;  
  else if( inbyte =='g' ) gyroscope = true; 
  else if( inbyte =='l' ) line = true;  
  else if( inbyte =='m' ) compass = true;  
  else if( inbyte =='s' ) sonar = true;  
  else if( inbyte =='t' ) temperature = true;  
}

void elabMotorCmd(int inbyte){
       if(inbyte == 45){       //45 is -
        min = 1;
      }else{
...
void loop()
{
  // explain();
   ...
   if( sonar )          readSonar();  
   if( line )           readLine();
   if( usingGY85 )      jobGY85();
   else if( using6050 ) job6050();
   //delay(200); //sends 5 info/sec
   delay(250);             // only read every 0,5 seconds, 10ms for 100Hz, 20ms for 50Hz
}
