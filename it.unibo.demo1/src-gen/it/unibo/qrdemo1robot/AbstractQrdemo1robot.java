...
class QaRobotActor extends it.unibo.qactor.robot.RobotActor{
	public QaRobotActor(
		...
		super(name, ctx, "./srcMore/it/unibo/qrdemo1robot/plans.txt", worldTheoryPath,
		outEnvView, it.unibo.qactor.robot.RobotSysKb.setRobotBase(ctx, baserobot) , defaultPlan);
	}
}

public class AbstractQrdemo1robot extends QaRobotActor { 
...
protected IActorAction  action;
	...
	public AbstractQrdemo1robot(String actorId, QActorContext myCtx, IOutputEnvView outEnvView ,String baserobot)  throws Exception{
		super(actorId, myCtx,  
		"./srcMore/it/unibo/qrdemo1robot/WorldTheory.pl",
		setTheEnv( outEnvView ) ,baserobot , "init");		
		this.planFilePath = "./srcMore/it/unibo/qrdemo1robot/plans.txt";
	...
	*/
    	...
    	nPlanIter++;
    		if( ! checkInMsgQueue() ){
    			//ReceiveMsg
    					 aar  = planUtils.receiveMsg(mysupport,
    					 "sonar" ,"MSGTYPE", 
    					 "qademo1sonara",this.getName(), 
    					 "sonar(sonara,qrdemo1robot,D)","MSGNUM", 20000, "" , "");	//could block
    					//println(getName() + " plan " + curPlanInExec  +  " interrupted=" + aar.getInterrupted() + " action goon="+aar.getGoon());
    					if( aar.getInterrupted() ){
    						curPlanInExec   = "receiveSonarADistance";
    						if( aar.getTimeRemained() <= 0 ) addRule("tout(ReceiveMsg,"+getName()+")");
    		...
    		if( currentMessage.msgId().equals("sonar") ){
    			...
    			/* Print */
    			parg =  updateVars( Term.createTerm("sonar(SONARNAME,TARGETNAME,DISTANCE)"), Term.createTerm("sonar(sonara,qrdemo1robot,F)"), 
    				    		  					Term.createTerm(currentMessage.msgContent()), parg);
    		...
    		if( currentMessage.msgId().equals("sonar") ){
    			String parg="distA(S)";
    			/* AddRule */
    			parg = updateVars(Term.createTerm("sonar(SONARNAME,TARGETNAME,DISTANCE)"),  Term.createTerm("sonar(sonara,qrdemo1robot,S)"), 
    				    		  					Term.createTerm(currentMessage.msgContent()), parg);
    		...
    		if( currentMessage.msgId().equals("sonar") ){
    			String parg = "";
    			/* SwitchPlan */
    			parg =  updateVars(  Term.createTerm("sonar(SONARNAME,TARGETNAME,DISTANCE)"), Term.createTerm("sonar(sonara,qrdemo1robot,D)"), 
    				    		  					Term.createTerm(currentMessage.msgContent()), parg);
    				if( parg != null ){
    	...
    	nPlanIter++;
    		if( ! checkInMsgQueue() ){
    			//ReceiveMsg
    					 aar  = planUtils.receiveMsg(mysupport,
    					 "sonar" ,"MSGTYPE", 
    					 "qademo1sonarb",this.getName(), 
    					 "sonar(sonarb,qrdemo1robot,D)","MSGNUM", 20000, "alarm" , "stopRobot");	//could block
    					//println(getName() + " plan " + curPlanInExec  +  " interrupted=" + aar.getInterrupted() + " action goon="+aar.getGoon());
    					if( aar.getInterrupted() ){
    						curPlanInExec   = "receiveSonarBDistance";
    						if( aar.getTimeRemained() <= 0 ) addRule("tout(ReceiveMsg,"+getName()+")");
    		...
    		if( currentMessage.msgId().equals("sonar") ){
    			...
    			/* Print */
    			parg =  updateVars( Term.createTerm("sonar(SONARNAME,TARGETNAME,DISTANCE)"), Term.createTerm("sonar(sonarb,qrdemo1robot,F)"), 
    				    		  					Term.createTerm(currentMessage.msgContent()), parg);
    				if( parg != null ) println( parg );  
    		}//onMsg
    		if( currentMessage.msgId().equals("sonar") ){
    ...
    while(true){
    	...
    	nPlanIter++;
    		...
    		println( temporaryStr );  
    		//forward
    		//if( ! execRobotMove("startRobot","forward",60,0,0, "alarm" , "stopRobot") ) break;
    		    aar = execRobotMove("startRobot","forward",60,0,0, "alarm" , "stopRobot");
    		    if( aar.getInterrupted() ){
    		    	curPlanInExec   = "startRobot";
    		    	if( ! aar.getGoon() ) break;
    		    } 			
    ...
    while(true){
    	...
    	nPlanIter++;
    		...
    		println( temporaryStr );  
    		//forward
    		//if( ! execRobotMove("leftRobot","forward",60,0,0, "alarm" , "stopRobot") ) break;
    		    aar = execRobotMove("leftRobot","forward",60,0,0, "alarm" , "stopRobot");
    		    if( aar.getInterrupted() ){
    		    	curPlanInExec   = "leftRobot";
    		    	if( ! aar.getGoon() ) break;
    		    } 			
    ...
    while(true){
    	...
    	nPlanIter++;
    		...
    		println( temporaryStr );  
    		//forward
    		//if( ! execRobotMove("rightRobot","forward",60,0,0, "alarm" , "stopRobot") ) break;
    		    aar = execRobotMove("rightRobot","forward",60,0,0, "alarm" , "stopRobot");
    		    if( aar.getInterrupted() ){
    		    	curPlanInExec   = "rightRobot";
    		    	if( ! aar.getGoon() ) break;
    		    } 			
    ...
    */
    protected void initSensorSystem(){		
    	try {
    		String goal = "consult( \"./src/it/unibo/qrdemo1robot/sensorTheory.pl\" )";
    ...
    */
    protected void addSensorObservers(){
    	for (ISensor<?> sensor : Configurator.getInstance().getSensors()) {
    		//println( "qrdemo1robot sensor= "  + sensor.getDefStringRep() );
    		//println( "qrdemo1robot sensor class= "  + sensor.getClass().getName() );
